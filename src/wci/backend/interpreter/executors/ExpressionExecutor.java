...
public class ExpressionExecutor extends StatementExecutor
{
	...
	/*	private Object executeComplex(ICodeNode node, ICodeNodeTypeImpl nodeType) {
			...
			ICodeNodeTypeImpl nodeType)
	{
		...
		//complex check
		SymTabEntry complexId1 = (SymTabEntry) operandNode1.getAttribute(ID);
		SymTabEntry complexId2 = (SymTabEntry) operandNode2.getAttribute(ID);

		TypeSpec complexType1 = complexId1.getTypeSpec();
		TypeSpec complexType2 = complexId2.getTypeSpec();

		if ((complexType1.getForm() == RECORD) || (complexType2.getForm()== RECORD)) {
			complexMode = true;
		}
